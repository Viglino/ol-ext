...
ol.source.HexBin = function(options) {
  options = options || {} ;
	...
	*/
	...
	this.resetBin();
	// Future features
	this._origin.on("addfeature", function(e){ this._onAddFeature(e.feature); }, this);
	this._origin.on("removefeature", function(e){ this._onRemoveFeature(e.feature); }, this);
};
ol.inherits (ol.source.HexBin, ol.source.Vector);
/**
 * Add a new feature to the source > add it to the original source
 * @param {ol.Feature} f 
 * /
ol.source.HexBin.prototype.addFeature = function(f) {
  this._origin.addFeature(f);
}
/**
 * Add a batch of features to the source. > add it to the original source
 * @param {Array.<ol.Feature>} f 
 * /
ol.source.HexBin.prototype.addFeatures = function(f) {
  this._origin.addFeatures(f);
}
/**
 * Remove a new feature to the source > remove it to the original source
 * @param {ol.Feature} f 
 * /
ol.source.HexBin.prototype.removeFeature = function(f) {
  this._origin.removeFeature(f);
}
/**
 * Remove all features from the source > remove it to the original source
 * @param {ol.Feature} f 
 * /
ol.source.HexBin.prototype.clear = function(opt) {
  this._origin.clear(opt);
}
/**
 ...
 * @private
 */
ol.source.HexBin.prototype._onAddFeature = function(f)
{	var h = this._hexgrid.coord2hex(this._geomFn(f));
    ...
    var ex = new ol.Feature(new ol.geom.Polygon([this._hexgrid.getHexagon(h)]));
		...
		this._bin[id] = ex;
		ol.source.Vector.prototype.addFeature.call(this,ex);
	}
	f.on("change", this.onModifyFeature, this);
};
 ...
 * Remove a feature 
 * @param {ol.Feature} f 
 * @param {ol.HexBin|undefeined} bin the bin, default get the feature bin
 */
ol.source.HexBin.prototype._onRemoveFeature = function(f, bin) {
  var b = bin || this.getBin(f);
	...
	f.un("change", this.onModifyFeature, this);
};
/**
 * Recalculate the bin on feature change
 * @param {*} e change event 
 */
ol.source.HexBin.prototype.onModifyFeature = function(e) {
  var bin = this.getBin(e.target);
	if (bin && bin.moved) {	
    // remove from the bin
		this._onRemoveFeature(e.target, bin);
		// insert in the new bin
		//addFeature.call (this, e.target);
		this._onAddFeature(e.target);
	}	
	this.changed();
};
// Clear all bins and generate a new one
ol.source.HexBin.prototype.resetBin = function() {
  this._bin = {};
	ol.source.Vector.prototype.clear.call(this);
	var features = this._origin.getFeatures();
	for (var i=0, f; f=features[i]; i++) {
    this._onAddFeature(f);
	};
};
/**
