 ...
 */
class ol_control_LayerSwitcher {
  constructor(options) {
    options = options || {};
    var self = this;
    this.dcount = 0;
  ...
  setMap(map) {
    ...
    if (this._listener) {
      ...
      if (this._listener)
        ol_Observable_unByKey(this._listener.size);
    }
    this._listener = null;
      ...
      if (target) {
        ...
        if (drop && target) {
          ...
          if (group)
            ...
            collection = self.getMap().getLayers();
  ...
  drawList(ul, collection) {
    ...
    for (var i = layers.length - 1; i >= 0; i--) {
      ...
      if (layer.getLayers) {
        ...
        li.classList.add('ol-layer-vector');
      else if (layer instanceof ol_layer_VectorTile)
        li.classList.add('ol-layer-vector');
      else if (layer instanceof ol_layer_Tile)
        li.classList.add('ol-layer-tile');
      else if (layer instanceof ol_layer_Image)
        li.classList.add('ol-layer-image');
      else if (layer instanceof ol_layer_Heatmap)
        li.classList.add('ol-layer-heatmap');
