...
class ol_style_FontSymbol {
  constructor(options) {
    ...
    if (options.glyph)
      ...
      this.glyph_ = this.getTextGlyph(options.text || '', options.font);
  ...
  getGlyph(name) {
    if (name)
      return ol_style_FontSymbol.prototype.defs.glyphs[name] || { font: 'sans-serif', char: name.charAt(0), theme: 'none', name: 'none', search: '' };
    else
      return this.glyph_;
  }
  ...
  getGlyphName() {
    for (var i in ol_style_FontSymbol.prototype.defs.glyphs) {
      if (ol_style_FontSymbol.prototype.defs.glyphs[i] === this.glyph_)
        return i;
    }
    return '';
  }
