 ...
 */
class ol_interaction_SnapGuides {
	constructor(options) {
		if (!options)
			options = {};
		// Intersect 2 guides
		...
		// Default style
		var sketchStyle = [new ol_style_Style({
    ...
    */
	addGuide(v, ortho) {
		if (v) {
			...
			var guideLength = Math.max(this.projExtent_[2] - this.projExtent_[0], this.projExtent_[3] - this.projExtent_[1]);
